...
FROM
	(Select * from wf_liveroom where Status < 2 order by RoomId Desc LIMIT :page*20,20) room
LEFT JOIN (
	SELECT
		a.LiveRoomCode,
...
module.exports = function({ sequelize, statistic, ctt, express, config, wrap, redisClient }) {
    ...
    router.get('/Home', wrap(async(req, res) => {
        ...
        let [AllColumn] = await sequelize.query("select Id,Name,concat(:picBaseURL, CoverImageUrl) Image from wf_video_column where IsDelete=0 order by SortOrder,Id desc", { replacements })
    ...
    router.get('/Teaching/:page', wrap(async(req, res) => {
        let [result] = await sequelize.query("select VideoCode Code,VideoName Title,concat(:picBaseURL, VideoImage)  Image,TimeLong ,DATE_FORMAT(ShowTime,'%Y-%m-%d %H:%i:%s') ShowTime from wf_live_video where Backup1=3 and Status=0 limit :page*20,20", { replacements: { page: Number(req.params.page), picBaseURL: config.picBaseURL } })
        res.send({ Status: 0, Explain: "", DataList: result })
    }));
    /**热门分类 */
    router.get('/Column', wrap(async(req, res) => {
        ...
        if (Column.length) {
            ...
            let [Detail] = await sequelize.query("select VideoCode Code,VideoName Title,concat(:picBaseURL, VideoImage)  Image,TimeLong,DATE_FORMAT(ShowTime,'%Y-%m-%d %H:%i:%s') ShowTime from wf_live_video where Status=0 and ColumnId=:Id order by ShowTime desc limit 20", { replacements: { picBaseURL: config.picBaseURL, Id: columnId } })
    ...
    router.get('/Dissertation/:Id/:page', wrap(async(req, res) => {
        let [Dissertation] = await sequelize.query("select VideoCode Code,VideoName Title,concat(:picBaseURL, VideoImage)  Image,TimeLong,DATE_FORMAT(ShowTime,'%Y-%m-%d %H:%i:%s') ShowTime from wf_live_video where Status=0 and DissertationId=:Id order by ShowTime desc limit :page*20,20", { replacements: { page: Number(req.params.page), picBaseURL: config.picBaseURL, Id: Number(req.params.Id) } })
        res.send({ Status: 0, Explain: "", DataList: Dissertation })
    }));
    /**最新视频列表 */
    router.get('/Lastest/:page', wrap(async(req, res) => {
        let [result] = await sequelize.query("select VideoCode Code,VideoName Title,concat(:picBaseURL, VideoImage) Image,TimeLong,DATE_FORMAT(ShowTime,'%Y-%m-%d %H:%i:%s') ShowTime from wf_live_video where Status=0 order by ShowTime desc limit :page*20,20", { replacements: { page: Number(req.params.page), picBaseURL: config.picBaseURL } })
        res.send({ Status: 0, Explain: "", DataList: result })
    }));
    return router;
}