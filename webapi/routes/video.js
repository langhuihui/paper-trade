...
module.exports = function({ sequelize, statistic, ctt, express, config, wrap, redisClient }) {
    ...
    router.get('/Teaching/:page', wrap(async(req, res) => {
        let [result] = await sequelize.query("select VideoCode Code,VideoName Title,concat(:picBaseURL, VideoImage)  Image,TimeLong ,DATE_FORMAT(ShowTime,'%Y-%m-%d %H:%i:%s') ShowTime from wf_live_video where Backup1=3 and Status=0 limit :page,20", { replacements: { page: Number(req.params.page) * 20, picBaseURL: config.picBaseURL } })
        res.send({ Status: 0, Explain: "", DataList: result })
    }));
    /**热门分类 */
    ...
    router.get('/Lastest/:page', wrap(async(req, res) => {
        let [result] = await sequelize.query("select VideoCode Code,VideoName Title,concat(:picBaseURL, VideoImage) Image,TimeLong,DATE_FORMAT(ShowTime,'%Y-%m-%d %H:%i:%s') ShowTime from wf_live_video where Status=0 order by ShowTime desc limit :page,20", { replacements: { page: Number(req.params.page) * 20, picBaseURL: config.picBaseURL } })
        res.send({ Status: 0, Explain: "", DataList: result })
    }));
    return router;
}