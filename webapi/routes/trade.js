...
module.exports = function({ config, mainDB, realDB, ctt, express, checkEmpty, mqChannel, redisClient, rongcloud, wrap }) {
    ...
    router.get('/RankList/:type', allowAccess(), async(req, res) => {
        switch (req.params.type) {
            ...
            case "TotalPrifitV": //炒股大赛总排行
                ...
                res.set('Content-Type', 'application/json').send({ Status: 0, Explain: "", DataList: matchTotalProfitReulst })
            ...
            case "WeekProfitV": //炒股大赛周排行
                ...
                res.set('Content-Type', 'application/json').send({ Status: 0, Explain: "", DataList: matchWeekProfitReulst })
    ...
    router.post('/QuotationUsRank/:ShowType/:Order/:Limit', ctt, wrap(async({ params: { ShowType, Order, Limit } }, res) => {
        ...
        switch (ShowType) {
            case "ALL":
                result = await realDB.collection(currentUSrankCname).find({}, { _id: 0, sort: { RiseFallRange: Order == "desc" ? -1 : 1 }, limit }).toArray()
                break
            case "CCS":
            case "GS":
            case "ETF":
                result = await realDB.collection(currentUSrankCname).find({ ShowType }, { _id: 0, sort: { RiseFallRange: Order == "desc" ? -1 : 1 }, limit }).toArray()
                break
        }
        res.send({ Status: 0, Explain: "", DataList: result })
    }));
